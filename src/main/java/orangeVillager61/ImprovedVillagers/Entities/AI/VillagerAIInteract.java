...
public class VillagerAIInteract extends EntityAIWatchClosest2
{
    /** The delay before the villager throws an itemstack (in ticks) */
    private int interactionDelay;
    private final IvVillager villager;

    public VillagerAIInteract(IvVillager villagerIn)
    {
        super(villagerIn, IvVillager.class, 3.0F, 0.02F);
    ...
    public void startExecuting()
    {
        ...
        if (this.villager.canAbondonItems() && this.closestEntity instanceof IvVillager && ((IvVillager)this.closestEntity).wantsMoreFood())
        {
            ...
            if (this.interactionDelay == 0)
            {
                ...
                for (int i = 0; i < inventorybasic.getSizeInventory(); ++i)
                {
                    ...
                    if (!itemstack.isEmpty())
                    {
                        ...
                        if ((item == Items.COOKED_BEEF || item == Items.COOKED_CHICKEN || item == Items.COOKED_PORKCHOP || item == Items.COOKED_MUTTON))
                        {
                            int l = itemstack.getCount() / 2;
                            itemstack.shrink(l);
                            itemstack1 = new ItemStack(item, l, itemstack.getMetadata());
